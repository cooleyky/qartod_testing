...
#
# @author: Kylene M. Cooley
#
# Revised 8 Aug 2025: Move M2M request steps to qc_completion
    ...
    for m in datastreams.index:
        ...
        if (grt_table is False) and (ct_table is False):
            ...
            try:
                ...
                test_exe, load_m2m = check_tests_exe(data, test_parameters,
                                                     grt_table, ct_table)
                # Make M2M request and load data if there are no attributes
                # for the QARTOD variables
                if load_m2m is True:
                    data = load_m2m_data(refdes, method, stream, deploy)
                    test_paramters = make_test_parameter_dict(data)
                    test_exe, load_m2m = check_tests_exe(
                        data, test_parameters, grt_table, ct_table)                                 
                ...
                del [grt_table, ct_table, data, test_parameters,
                     test_exe, table]#!/usr/bin/env qartod_test
#
# run_qartod_test_cross-ref.py
#
# @author: Kylene M. Cooley
